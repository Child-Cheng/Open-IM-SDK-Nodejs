...
export default class OpenIMSDK extends Emitter {
  private ws: w3cwebsocket | undefined | WebSocket;
  ...
  private wsSend = (
    ...
    const handleMessage = (ev: IMessageEvent) => {
  ...
  private getPlatform() {
    const wflag = typeof w3cwebsocket;
    //@ts-ignore
  ...
  private createWs(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (this.platform === 'web') {
        console.log('web ws...', this.wsUrl);
        this.ws = new w3cwebsocket(this.wsUrl); //typeof window !== 'undefined'? new WebSocket(this.wsUrl): new w3cwebsocket(this.wsUrl);
